...
BOOL authenticate_user(WCHAR* wDomain, WCHAR* wUsername, WCHAR* wPassword)
{
    ...
    clientToServerToken = (PBYTE)KERNEL32$HeapAlloc(
    ...
    serverToClientToken = (PBYTE)KERNEL32$HeapAlloc(
        KERNEL32$GetProcessHeap(),
        HEAP_ZERO_MEMORY,
        MAX_TOKEN_SIZE
    );
    ...
    secStatus = SECUR32$GetUserNameExW(
        NameSamCompatible,
        serverPrincipalName,
    ...
    while (clientContinue || serverContinue)
    {
        if (clientContinue)
        {
            ...
            secStatus = SECUR32$InitializeSecurityContextW(
                ...
                clientInputBuffer,
                0,
                clientContextHandleOut,
                clientOutputBuffer,
                &clientContextAttributesOut,
                &clientContextExpiry
            );
        ...
        if (serverContinue)
        {
            ...
            secStatus = SECUR32$AcceptSecurityContext(
                &serverCredHandle,
                serverContextHandleIn,
                serverInputBuffer,
                serverContextAttributes,
                SECURITY_NATIVE_DREP,
                serverContextHandleOut,
                serverOutputBuffer,
                &serverContextAttributesOut,
                &serverContextExpiry
            );
...
int go(char *args, int len)
{
	...
	if(iBytesLen != 0)
    {
        ...
        {
			internal_printf(
                "[*] Authenticated to Domain Controller: %S\n"
                "============================================================\n\n",
